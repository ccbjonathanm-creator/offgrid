/* ============================================================
   ...
   fois, elle est chiffrée (AES-GCM + passphrase) et gardée sur son
   seul appareil. Rien n'est déployé de sensible.
   ============================================================ */
const Vendeur = (() => {

  const VKEY = 'offgrid.vendeur';   // blob chiffré {salt, iv, ct}
  let privInMemory = null;          // clé privée déchiffrée, en mémoire de session seulement
  ...
  async function storePriv(jwkStr, pass){
    JSON.parse(jwkStr); // valide le JSON, throw si invalide
  ...
  function viewSetup(body, close){
    body.innerHTML = `
      ...
      <div class="btn-row"><button class="btn ghost" id="v-cancel">Annuler</button><button class="btn" id="v-save">Enregistrer (chiffré)</button></div>`;
    ...
    body.querySelector('#v-save').addEventListener('click', async ()=>{
      ...
      try{ await storePriv(priv, pass); privInMemory = priv; App.toast('Clé vendeur enregistrée 🔒'); viewGenerate(body, close); }
  ...
  function viewUnlock(body, close){
    body.innerHTML = `
      ...
      <label class="field"><span class="lab">Passphrase vendeur</span>
        <input type="password" id="v-pass" placeholder="mot de passe vendeur"></label>
      <div id="v-status" class="hint"></div>
      <div class="btn-row"><button class="btn ghost" id="v-forget">Oublier la clé</button><button class="btn" id="v-unlock">Déverrouiller</button></div>`;
    body.querySelector('#v-forget').addEventListener('click', ()=>{
      if (confirm('Supprimer la clé vendeur de cet appareil ?')){ localStorage.removeItem(VKEY); privInMemory=null; close(); App.toast('Clé vendeur supprimée'); }
    });
  ...
  function viewGenerate(body, close){
    body.innerHTML = `
      ...
      <div class="btn-row" style="justify-content:space-between">
        ...
        <button class="btn" id="v-gen">Générer la clé</button></div>
  ...
  // appui long sur un élément -> ouvre le mode vendeur
